...
$(document).ready(function () {
    ...
    if (mobile_mode) {
        ...
        dtable = $('#data_table_mobile').dataTable({
            ...
            "iDisplayLength": 10,
            "bAutoWidth": false,
            "sAjaxSource": "/admin/shop/data/list?mobile=true",
        ...
        dtable = $('#data_table').dataTable({
            ...
            "iDisplayLength": 10,
            "bAutoWidth": true,
            "sAjaxSource": "/admin/shop/data/list",
            "fnServerData": function ( sSource, aoData, fnCallback ) {
                $.ajax( {
                    "dataType": 'json',
                    "type": "GET",
...
$('#filename').change(function () {
    ...
    var url = 'http://' + lang + host + '/shop/' + gid + '/' + path;
...
function submitEdit(value, settings) {
    ...
    $.ajax({
        type: 'POST',
        url: '/admin/shop/data/save/field',
        data: {
            field_name: arr[0],
            field_id: arr[1],
            field_value: value
            ...
            success: function (data) {
                ...
                if (data.result == '0') {
                    ...
                    if (data.id && flag_open_upload_after_save == 1) {
                        ...
                        imgbrowser = open('/admin/shop/images?dir=' + data.id, 'displayWindow', features);
                    }
                    if (edit_id == 0) {
...
function () {
    ...
    features = 'top=' + top + ',left=' + left;
    features += ',height=' + h + ',width=' + w + ',resizable=no';
    imgbrowser = open('/admin/shop/images?dir=' + edit_id, 'displayWindow', features);
});
...
$('#eb_switch_cm').click(function () {
    ...
    if (!confirm(js_lang_switch_mode_question)) {
        return;
    }
    if (edit_id == 0) {
        $('#wysiwyg_editor_wrap').hide();
