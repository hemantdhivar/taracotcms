...
sub _load_lang {
  ...
  my $lang_adm = YAML::XS::LoadFile(config->{root_dir}.'lib/modules/shop/lang/en.lng') || {};
  my $lang_adm_cnt={};
  if ($lng ne 'en') {
   $lang_adm_cnt = YAML::XS::LoadFile(config->{root_dir}.'lib/modules/shop/lang/'.$lng.'.lng') || {};
  }
...
sub generate_special_html {
 ...
 $sth = database->prepare(
   'SELECT id,pagetitle,filename,groupid,description_short FROM '.config->{db_table_prefix}.'_shop WHERE status=1 AND special_flag=1 AND lang='.database->quote($current_lang)
 );
 if ($sth->execute()) {
  while(my ($id,$pagetitle,$filename,$groupid,$description_short) = $sth -> fetchrow_array) {
    my $picture;
    if (-d config->{files_dir}.'/images_shop/'.$id) {
      opendir(DIR, config->{files_dir}.'/images_shop/'.$id) || die "Fatal error: can't open directory\n";
      while((my $filename = readdir(DIR))) {
        if ($filename =~  m/^\.\.?/) {
          next;
         }
         $picture="/files/images_shop/$id/.".md5_hex($filename).".jpg";
         last;
      }
      closedir(DIR);       
    } # if dir exists
    ...
    $items .= template 'shop_special_item', { lang => $lang, picture => $picture, id => $id,  pagetitle => $pagetitle, filename => $filename, groupid => $groupid, description_short => $description_short }, { layout => undef };
  } # while     
 } 
 $sth->finish();  
 my $tmp = template 'shop_special', { items => $items }, { layout => undef };
...
get qr{(.*)} => sub {
  ...
  if (!$url || !$groupid) {
    ...
    if ($groupid) {
      $sth = database->prepare(
       'SELECT id,pagetitle,filename,groupid,description_short FROM '.config->{db_table_prefix}.'_shop WHERE status=1 AND groupid='.database->quote($groupid).' AND lang='.database->quote($_current_lang)
      );
    } else {
      $sth = database->prepare(
       'SELECT id,pagetitle,filename,groupid,description_short FROM '.config->{db_table_prefix}.'_shop WHERE status=1 AND lang='.database->quote($_current_lang)
      );
    }
    my $items;
    if ($sth->execute()) {
     while(my ($id,$pagetitle,$filename,$groupid,$description_short) = $sth -> fetchrow_array) {
       ...
       $items .= template 'shop_item', { detect_lang=> $detect_lang, lang => $lang, picture => $picture, id => $id,  pagetitle => $pagetitle, page_data => $page_data, filename => $filename, groupid => $groupid, description_short => $description_short }, { layout => undef };
     } # while     
    }
    $sth->finish();
    if (!$items) {
    ...
    $db_data{'content'} = $db_var1->{s_value_html}.template 'shop_list', { items => $items }, { layout => undef };
    my $render_template = &taracot::_process_template( template 'shop_view_index', { detect_lang=> $detect_lang, current_lang => $_current_lang, lang => $lang, auth_data => $auth_data, site_title => $stitle->{s_value}, page_data => $page_data, db_data => \%db_data, pagetitle => $db_var1->{s_value} || $lang->{module_name} }, { layout => config->{layout}.'_'.$_current_lang } );
    if ($render_template) {
  ...
  my $db_data = database->quick_select(config->{db_table_prefix}.'_shop', { groupid => $groupid, filename => $url, lang => $_current_lang });
  if (defined $db_data && $db_data->{id}) {
   my $render_template; 
   if ($db_data->{status} eq 1) {       
    my %img_pics;
    my %img_urls;  
    if (-d config->{files_dir}.'/images_shop/'.$db_data->{id}) {
      opendir(DIR, config->{files_dir}.'/images_shop/'.$db_data->{id}) || die "Fatal error: can't open directory\n";
      my $cnt=1;
      while((my $filename = readdir(DIR))) {
       if ($filename =~  m/^\.\.?/) {
        next;
       }
       $img_pics{$cnt} = "/files/images_shop/".$db_data->{id}."/.".md5_hex($filename).".jpg";
       $img_urls{$cnt} = "/files/images_shop/".$db_data->{id}."/".$filename;
       $cnt++;
      }
      closedir(DIR);
    }
    ...
    my $grid=template 'admin_shop_images_grid', { detect_lang=> $detect_lang, id => $db_data->{id}, img_pics => \%img_pics, img_urls => \%img_urls }, { layout => undef };
    my $db_var1  = database->quick_select(config->{db_table_prefix}.'_settings', { s_name => "shop_title_".$groupid, lang => $_current_lang });
    my $groupid_title = $db_var1->{s_value} || $groupid;
    $render_template = &taracot::_process_template( template 'shop_view', { detect_lang=> $detect_lang, current_lang => $_current_lang, lang => $lang, auth_data => $auth_data, site_title => $stitle->{s_value}, page_data => $page_data, db_data => $db_data, grid => $grid, url => $url, groupid => $groupid, groupid_title => $groupid_title, pagetitle => $db_data->{pagetitle} }, { layout => $db_data->{layout}.'_'.$db_data->{lang} } );
   }
   if ($db_data->{status} eq 0) {
    $render_template = &taracot::_process_template( $taracot::taracot_render_template = template 'shop_status', { detect_lang=> $detect_lang, site_title => $stitle->{s_value}, page_data => $db_data, status_icon => "disabled_32.png", status_header => $lang->{disabled_header}, status_text => $lang->{disabled_text} }, { layout => $db_data->{layout}.'_'.$db_data->{lang} } );
   }
   if ($db_data->{status} eq 2) {
    $render_template = &taracot::_process_template( $taracot::taracot_render_template = template 'shop_status', { detect_lang=> $detect_lang, site_title => $stitle->{s_value}, page_data => $db_data, status_icon => "under_construction_32.png", status_header => $lang->{construction_header}, status_text => $lang->{construction_text} }, { layout => $db_data->{layout}.'_'.$db_data->{lang} } );
   }
   if ($render_template) {
    return $render_template;
   }
...
get '/' => sub {
  ...
  $hash_langs=~s/, //;
  return template 'admin_shop_index', { current_lang => $_current_lang, default_lang => config->{lang_default}, lang => $lang, navdata => $navdata, authdata => $auth, list_layouts => $list_layouts, list_langs => $list_langs, hash_langs => $hash_langs }, { layout => 'admin' };
...
get '/data/list' => sub {
  ...
  my $sth = database->prepare(
   'SELECT COUNT(*) AS cnt FROM '.config->{db_table_prefix}.'_shop WHERE 1'
  );
  if ($sth->execute()) {
  ...
  if ($where ne '1' && $total > 0) {
   my $sth = database->prepare(
    'SELECT COUNT(*) AS cnt FROM '.config->{db_table_prefix}.'_shop WHERE '.$where
   );
   if ($sth->execute()) {
  ...
  $columns=~s/,$//;
  $sth = database->prepare(
   'SELECT '.$columns.' FROM '.config->{db_table_prefix}.'_shop WHERE '.$where.$sortorder.' LIMIT '.$iDisplayStart.', '.$iDisplayLength
  );
  if ($sth->execute()) {
...
post '/data/save' => sub {
  ...
  my $sth = database->prepare(
   'SELECT id FROM '.config->{db_table_prefix}.'_shop WHERE filename='.database->quote($filename).$dupesql
  );
  if ($sth->execute()) {
  ...
  if ($id > 0) {
   database->quick_update(config->{db_table_prefix}.'_shop', { id => $id }, { pagetitle => $pagetitle, filename => $filename, groupid => $groupid, keywords => $keywords, description => $description, description_short => $description_short, status => $status, content => $content, lang => $plang, layout => $layout, special_flag => $special_flag, lastchanged => time });
  } else {   
   database->quick_insert(config->{db_table_prefix}.'_shop', { pagetitle => $pagetitle, filename => $filename, groupid => $groupid, keywords => $keywords, description => $description, description_short => $description_short,  status => $status, content => $content, lang => $plang, layout => $layout, special_flag => $special_flag, lastchanged => time });
...
post '/data/save/field' => sub {
  ...
  if ($field_name eq 'lang') {
   ...
   my $filename;
   $sth = database->prepare(
    'SELECT filename FROM '.config->{db_table_prefix}.'_shop WHERE id = '.$field_id
   );
   if ($sth->execute()) {
   ...
   $sth = database->prepare(
    'SELECT id FROM '.config->{db_table_prefix}.'_shop WHERE filename = '.database->quote($filename).' AND lang = '.database->quote($field_value)
   );
   if ($sth->execute()) {
    my ($tmpid) = $sth->fetchrow_array;
    if ($tmpid) {
   ...
   $sth->finish();
    database->quick_update(config->{db_table_prefix}.'_shop', { id => $field_id }, { lang => $field_value, lastchanged => time });
    return '{"result":"1"}';
   }
  ...
  if ($field_name eq 'layout') {
   ...
   database->quick_update(config->{db_table_prefix}.'_shop', { id => $field_id }, { layout => $field_value, lastchanged => time });
   return '{"result":"1"}';
  } 
  ...
  if ($field_name eq 'groupid') {
    ...
    my $page_data  = database->quick_select(config->{db_table_prefix}.'_shop', { id => $field_id });
    my $sth = database->prepare(
     'SELECT id FROM '.config->{db_table_prefix}.'_shop WHERE filename='.database->quote($page_data->{filename}).' AND id != '.$field_id.' AND lang='.database->quote($page_data->{lang}).' AND groupid='.database->quote($groupid)
    );
    if ($sth->execute()) {
     my ($tmpid) = $sth->fetchrow_array;
     if ($tmpid) {
    ...
    $sth->finish();
    database->quick_update(config->{db_table_prefix}.'_shop', { id => $field_id }, { groupid => $groupid, lastchanged => time });
    return '{"result":"1"}';
  } 
  ...
  if ($field_name eq 'pagetitle') {
   ...
   database->quick_update(config->{db_table_prefix}.'_shop', { id => $field_id }, { pagetitle => $pagetitle, lastchanged => time });
   return '{"result":"1"}';
  }
  ...
  if ($field_name eq 'status') {
   ...
   database->quick_update(config->{db_table_prefix}.'_shop', { id => $field_id }, { status => $status, lastchanged => time });
   return '{"result":"1"}';
  }
  ...
  if ($field_name eq 'filename') {
   ...
   my $sth = database->prepare(
    'SELECT id FROM '.config->{db_table_prefix}.'_shop WHERE filename='.database->quote($filename).' AND id != '.$field_id.' AND lang='.database->quote($page_data->{lang}).' AND groupid='.database->quote($page_data->{groupid})
   );
   if ($sth->execute()) {
    my ($tmpid) = $sth->fetchrow_array;
    if ($tmpid) {
   ...
   $sth->finish();
   database->quick_update(config->{db_table_prefix}.'_shop', { id => $field_id }, { filename => $filename, lastchanged => time });
...
post '/data/load' => sub {  
  ...
  my $db_data  = database->quick_select(config->{db_table_prefix}.'_shop', { id => $id });
...
post '/data/delete' => sub {
  ...
  if(ref($id) eq 'ARRAY'){
   foreach my $item (@$id) {
    $item=int($item);
    $del_sql.=' OR id='.$item;
    if (-d config->{files_dir}.'/images_shop/'.$item) {
      removeDir(config->{files_dir}.'/images_shop/'.$item);
    }
   }
   if ($del_sql) {
    $del_sql=~s/ OR //;    
    ...
    $del_sql='id='.int($id);
    if (-d config->{files_dir}.'/images_shop/'.int($id)) {
      removeDir(config->{files_dir}.'/images_shop/'.int($id));
    }
  }
  
  if ($del_sql) {
    my $sth = database->prepare(
     'DELETE FROM '.config->{db_table_prefix}.'_shop WHERE '.$del_sql
    );
    my $res;
    if ($sth->execute()) {
     $res=qq~{"result":"1"}~;
...
get '/images' => sub {             
  ...
  return template 'admin_shop_images', { lang => $lang, pagetitle => $lang->{pagetitle}, files_url => config->{files_url}, dir => $dir, pagetitle => $lang->{imgbrowser} }, { layout => 'browser' };
};
...
get '/data/images/dirdata' => sub {
  ...
  if (!-d config->{files_dir}.'/images_shop/'.$dir) {
   makePath(config->{files_dir}.'/images_shop/'.$dir);
  }
  my %resp = ( 
   current_dir => $dir,
   files => getDir(config->{files_dir}.'/images_shop/'.$dir, $dir)  
  );  
  my $json_xs = JSON::XS->new();
...
post '/data/images/upload' => sub {
  ...
  $fn=unidecode($fn);
  $fn=~s/[^a-zA-Z0-9\-_\.]//gm;
  my $fp=config->{files_dir}."images_shop/".$dir.'/'.$fn;
  $fp=~s/\/\//\//gm;
  $file->copy_to($fp);
  if (-e $fp) {
   ...
   $img->write(file => config->{files_dir}."images_shop/".$dir.'/.'.md5_hex($fn).'.jpg');
  }
...
post '/data/images/delete' => sub {
  ...
  content_type 'application/json';
  my $dir=param('dir');
  if ($dir !~ /^[0-9]{0,80}$/ || !int($dir) || !-d config->{files_dir}."images_shop/".$dir) {
  ...
  foreach my $delete_file (@files) {
   ...
   if ($delete_file !~ /^[\.A-Za-z0-9_\-]{1,100}$/ || $delete_file =~ m/\.\./ || !-e config->{files_dir}.'images_shop/'.$dir.'/'.$delete_file) {
    return '{"status":"0","reason":"'.$lang->{file_syntax_error}.'","file":"'.$delete_file.'"}';
   }
   my $res=undef;
   $res=removeFile(config->{files_dir}."images_shop/".$dir.'/'.$delete_file);
   my $md5n=md5_hex($delete_file);
   if (-e config->{files_dir}."images_shop/".$dir.'/.'.$md5n.'.jpg') {
    $res=removeFile(config->{files_dir}."images_shop/".$dir.'/.'.$md5n.'.jpg');
   }  
   if (!$res) {
...
post '/data/images/rename' => sub {
  ...
  if ($old_name !~ /^[\.A-Za-z0-9_\-]{1,100}$/ || $old_name =~ m/\.\./ || !-e config->{files_dir}."images_shop/".$dir.'/'.$old_name) {
  ...
  my $res=undef;
  $res=moveFile(config->{files_dir}."images_shop/".$dir.'/'.$old_name, config->{files_dir}."images_shop/".$dir.'/'.$new_name);
  my $md5n=md5_hex($old_name);
  if (-e config->{files_dir}."images_shop/".$dir.'/.'.$md5n.'.jpg') {
   $res=moveFile(config->{files_dir}."images_shop/".$dir.'/.'.$md5n.'.jpg', config->{files_dir}."images_shop/".$dir.'/.'.md5_hex($new_name).'.jpg');
  }  
  if (!$res) {
