...
get '/' => sub {  
  ...
  if (!$lf) {
    return &taracot::_process_template( template ('portfolio_error', { detect_lang => $detect_lang, head_html => '<link href="'.config->{modules_css_url}.'portfolio.css" rel="stylesheet" /><link href="/js/slimbox/css/slimbox2.css" rel="stylesheet" />', lang => $lang, page_data => $page_data, pagetitle => $lang->{module_name}, auth_data => $auth_data, err_msg => $lang->{id_error} }, { layout => config->{layout}.'_'.$_current_lang }), $auth_data );
  }
  my $pf=from_json $$lf;
  my $data_items='';  
